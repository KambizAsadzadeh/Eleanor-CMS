	...
	public static function ShowError($a,$info,$captcha)
	{
		...
		elseif($a['mail'])
			$tosend='<hr />'.($info['error'] ? Eleanor::$Template->Message($info['error'],'error') : '')
				.'<form method="post">'
				.(Eleanor::$Login->IsUser() ? '' : '<div class="errorinput"><span>'.$lang['yourname'].'</span><br />'.Eleanor::Input('name',$info['name']).'</div><br />')
				.'<div class="errorinput"><span>'
				.$lang['tell_us'].'</span><br />'.$GLOBALS['Eleanor']->Editor->Area('text',$info['text']).'</div>'
				.($info['back'] ? Eleanor::Input('back',$info['back'],array('type'=>'hidden')) : '')
				.($captcha ? '<br /><div class="errorinput"><span>'.$lang['captcha'].'</span><br /><span class="small">'.$lang['captcha_'].'</span><br />'.Eleanor::Input('check','',array('autocomplete'=>'off')).'<br />'.$captcha.'</div>' : '')
