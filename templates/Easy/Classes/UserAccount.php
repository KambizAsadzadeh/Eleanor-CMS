	...
	*/
	public static function AcRegister($values,$captcha,$errors)
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			...
			->begin()
			->item(array($lang['name'],Eleanor::Input('name',$values['name'],array('tabindex'=>1,'style'=>'width:80%','placeholder'=>$lang['enter_g_name'])).' <a href="#" title="'.$lang['check'].'"><img src="'.Eleanor::$Template->default['theme'].'images/no_dublicate.png" alt="" /></a><div id="name-error" style="color:red;display:none;">'.$errname.'</div>','tip'=>$lang['name_'],'imp'=>true,'td1'=>array('style'=>'width:150px;')))
			->item($lang['full_name'],Eleanor::Input('full_name',$values['full_name'],array('tabindex'=>2,'style'=>'width:80%')))
			->item(array('E-mail',Eleanor::Input('email',$values['email'],array('tabindex'=>3,'style'=>'width:80%','placeholder'=>$lang['enter_g_email'])).' <a href="#" title="'.$lang['check'].'"><img src="'.Eleanor::$Template->default['theme'].'images/no_dublicate.png" alt="" /></a><div id="email-error" style="color:red;display:none;">'.$erremail.'</div>','tip'=>$lang['email_'],'imp'=>true))
			->item(array($lang['pass'],Eleanor::Input('password',$values['password'],array('type'=>'password','tabindex'=>4)).'<div id="password-error" style="color:red;display:none;">'.$errpass.'</div>','tip'=>$lang['pass_']))
			->item($lang['rpass'],Eleanor::Input('password2',$values['password2'],array('type'=>'password','tabindex'=>5)).'<div id="password2-error" style="color:red;display:none;">'.$lang['PASSWORD_MISMATCH'].'</div>');
		if($captcha)
			$Lst->item(array($lang['captcha'],$captcha.'<br />'.Eleanor::Input('check','',array('tabindex'=>6,'autocomplete'=>'off')),'tip'=>$lang['captcha_']));
	...
	*/
	public static function AcRemindPass($values,$captcha,$errors)
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			...
			->item(array($lang['enterna'],Eleanor::Input('name',$values['name'],array('tabindex'=>1)).'<br /><a href="#" class="small">'.$lang['fogotname'].'</a>','tr'=>array('id'=>'tr-name','style'=>$em ? 'display:none' : ''),'td1'=>array('style'=>'width:170px')))
			->item(array($lang['enterem'],Eleanor::Input('email',$values['email'],array('tabindex'=>2)).'<br /><a href="#" class="small">'.$lang['fogotemail'].'</a>','tr'=>array('id'=>'tr-email','style'=>$em ? '' : 'display:none'),'td1'=>array('style'=>'width:170px')));
		if($captcha)
	...
	*/
	public static function AcRemindPassStep3($values,$captcha,$errors=array())
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			...
			->begin()
			->item(array($lang['ent_newp'],Eleanor::Input('password',$values['password'],array('type'=>'password','tabindex'=>1)).'<div id="password-error" style="color:red;display:none;">'.$errpass.'</div>','tip'=>$lang['pass_'],'td1'=>array('style'=>'width:200px')))
			->item($lang['rep_newp'],Eleanor::Input('password2',$values['password2'],array('type'=>'password','tabindex'=>2)).'<div id="password2-error" style="color:red;display:none;">'.$lang['PASSWORD_MISMATCH'].'</div>');
		if($captcha)
			$Lst->item(array($lang['captcha'],$captcha.'<br />'.Eleanor::Input('check','',array('tabindex'=>3,'autocomplete'=>'off')),'descr'=>$lang['captcha_']));
	...
	*/
	public static function AcReactivation($sent,$captcha,$errors,$hours)
	{
		...
		else
		{
			...
			if($captcha)
				$Lst->item(array($lang['captcha'],$captcha.'<br />'.Eleanor::Input('check','',array('tabindex'=>1,'autocomplete'=>'off')),'descr'=>$lang['captcha_']));
	...
	*/
	public static function AcEmailChange($values,$captcha,$errors)
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			->form()
			->begin()
			->item(array($lang['curr_email'],($em=Eleanor::$Login->Get('email')) ? $em : '&mdash;','td1'=>array('style'=>'width:200px')))
			->item($lang['new_email'],Eleanor::Input('email',$values['email'],array('tabindex'=>1)));
	...
	*/
	public static function AcNewPass($success,$errors,$values)
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			...
			->item(array($lang['en_ycp'],Eleanor::Input('old',$values['old'],array('type'=>'password','tabindex'=>1)),'td1'=>array('style'=>'width:200px')))
			->head($lang['new_pass_me'])
			->item($lang['ent_newp'],Eleanor::Input('password','password',$values['password'],array('type'=>'password','tabindex'=>2)).'<div id="password-error" style="color:red;display:none;">'.$errpass.'</div>')
			->item($lang['rep_newp'],Eleanor::Input('password2',$values['password2'],array('type'=>'password','tabindex'=>3)).'<div id="password2-error" style="color:red;display:none;">'.$lang['PASSWORD_MISMATCH'].'</div>')
	...
	*/
	...
	public static function AcOptions($controls,$values,$avatar,$errors,$saved)
	{
		...
		$Lst=Eleanor::LoadListTemplate('table-form')
			...
			->item(
				$lang['amanage'],
				Eleanor::Input('avatar_location',$values['avatar_location'],array('type'=>'hidden','id'=>'avatar-input'))
				.'<div id="avatar-local">
...
<img src="http://loginza.ru/img/providers/webmoney.png" title="WebMoney" /></a><br />'.($s ? '<div id="externals">'.rtrim($s,', ').'</div><br />' : '').'<a href="https://loginza.ru/api/widget?token_url='.urlencode($links['return']).'" class="loginza link-button" style="width:150px"><b>'.$lang['add'].'</b></a></div>';
	}
