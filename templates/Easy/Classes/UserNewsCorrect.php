...
class TplUserNewsCorrect
{
	...
	*/
	...
	/*
		Страница создания/редактирования новости
	...
	*/
	public static function AddEdit($id,$values,$errors,$uploader,$voting,$bypost,$hasdraft,$back,$links,$captcha)
	{
		...
		if($GLOBALS['Eleanor']->Categories->dump)
			$Lst->item(static::$lang['categs'],Eleanor::Items('cats',$GLOBALS['Eleanor']->Categories->GetOptions($values['cats']),array('id'=>'cs','tabindex'=>3,'size'=>10)))
		...
		if($captcha)
			$Lst->item(array(static::$lang['captcha'],$captcha.'<br />'.Eleanor::Input('check','',array('tabindex'=>13,'autocomplete'=>'off')),'descr'=>static::$lang['captcha_']));
