...
if(is_array($tabs))
{
	...
	foreach($tabs as $k=>$v)
	{
		...
		$double=Eleanor::$langs[$k]['d'];

		echo<<<HTML
<div id="{$id}" class="tab-pane{$active}" lang="{$double}">{$v}</div>
HTML;
		...
		$flags.=<<<HTML
<li{$active} lang="{$double}"><a href="#{$id}" class="{$k}" id="a-{$id}" data-toggle="tab" data-language="{$k}" title="{$name}"><img src="{$f_path}{$k}.png" alt="" /></a></li>
HTML;
	}
