	...
	public static function AddEdit($id,$values,$runned,$uploader,$links,$errors,$bypost,$back)
	{
		...
		if(Eleanor::$vars['multilang'])
		{
			...
			foreach(Eleanor::$langs as $k=>&$v)
			{
				...
				$ml['text'][$k]=$GLOBALS['Eleanor']->Editor->Area('text['.$k.']',Eleanor::FilterLangValues($values['text'],$k),array('post'=>$bypost,'no'=>array('tabindex'=>19)));
			}
		}
		else
			$ml=array(
				...
				'text'=>$GLOBALS['Eleanor']->Editor->Area('text',$values['text'],array('post'=>$bypost,'no'=>array('tabindex'=>19))),
			);
