...
class ContextLinks extends \Eleanor\BaseClass implements CMS\Interfaces\HtmlParser
{
	...
	public static function Parse($s)
	{
		...
		if($links===false)
		{
			...
			$R=Eleanor::$Db->Query('SELECT `from`,`regexp`,`to`,`url`,`params`,`date_from`,`date_till` FROM `'
				.CMS\P.'context_links` INNER JOIN `'.CMS\P.'context_links_l` USING(`id`) WHERE `language` IN (\'\',\''
				.Language::$main.'\') AND `status`=1');
