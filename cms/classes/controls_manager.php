...
class Controls_Manager extends Controls
{
	...
	public function ConfigureControl(array$co=array(),$ajax=false,$onlyprev=false)
	{
		...
		if($co['post'])
		{
			$this->name=$co['controls_name'];
			$co['type']=$this->SaveControl(array('type'=>'select','name'=>'type'));
		}
		elseif($ajax)
			$co['post']=true;

		$this->name=$co['settings_name'];
			...
			try
			{
				if($this->langs)
				{
					...
					*/
					...
					$co['options']=$options;
					$result['preview']=$this->DisplayControls(array('preview'=>array('type'=>$co['type'],'post'=>$co['post'],'load_eval'=>$co['load_eval'],'multilang'=>true)),array('preview'=>array('value'=>$co['default'],'options'=>$co['options'])));
					$result['preview']=isset($result['preview']['preview']) ? $result['preview']['preview'] : '';
				}
				else
					$result['preview']=$this->DisplayControl(array('type'=>$co['type'],'name'=>'preview','post'=>$co['post'],'options'=>$co['options'],'value'=>$co['default'],'load_eval'=>$co['load_eval'],'multilang'=>false));
			}
			catch(EE $E)
			{
				$error=$E->getMessage();
		...
		if(!$ajax)
		{
			...
			$result['type']=$this->DisplayControl(array('type'=>'select','name'=>'type','post'=>$co['post'],'value'=>$co['type'],'options'=>array('options'=>$options,'extra'=>array('id'=>'type-selector','onchange'=>'EC.ChangeType()','style'=>'width:80%'))));
		}
		$this->name=$oldname;
