...
#ToDo! keyword теперь имеют вид ,kw1,,kw2,,kw3,
class Settings extends \Eleanor\BaseClass
{
	public
	...
	final public function __construct()
	{
		...
		if($fatal)
		{
			...
			foreach(Eleanor::$services as &$v)
				...
				{
					rename(Eleanor::$root.$v['file'],Eleanor::$root.basename($v['file'],'.php').'.bak');
	...
	*/
	...
	protected function ShowListOptions(array$a,$gshow=true,$group=0,$error='')
	{
		...
		foreach($a as &$v)
		{
			...
			$controls[$v['name']]=array(
				...
				'_adelete'=>!$v['protected'] && $this->opts_mod ? $El->Url->Construct(array($this->pp.'odelete'=>$v['id'])) : false,
	...
	protected function AddEditGroup($id,$errors=array())
	{
		...
		else
		{
			...
			$values['title']=$values['descr']=Eleanor::$vars['multilang'] ? array_fill_keys(array_keys(Eleanor::$langs),'') : '';
			if(Eleanor::$vars['multilang'])
			{
				$values['_onelang']=true;
		...
		if($errors)
		{
			if($errors===true)
				$errors=array();
			$bypost=true;
			$values['pos']=isset($_POST['pos']) ? (string)$_POST['pos'] : 1;
			if(Eleanor::$vars['multilang'])
			{
				...
				foreach(Eleanor::$langs as $k=>$v)
				{
					...
					$values['descr'][$k]=isset($_POST['descr'][$k]) ? (string)$_POST['descr'][$k] : '';
				}
			}
			else
			{
			...
			else
			{
				...
				$values['keyword']=isset($_POST['keyword']) ? (string)$_POST['keyword'] : '';
	...
	*/
	...
	protected function AddEditOption($id,$errors=array(),$group=0)
	{
		...
		else
		{
			...
			$values['title']=$values['descr']=$values['value']=$values['serialized']=$values['default']=$values['startgroup']=Eleanor::$vars['multilang'] ? array_fill_keys(array_keys(Eleanor::$langs),'') : '';
			if(Eleanor::$vars['multilang'])
			{
				$values['_onelang']=true;
				$values['_langs']=array_keys(Eleanor::$langs);
		...
		if($errors)
		{
			...
			if(Eleanor::$vars['multilang'])
			{
				...
				foreach(Eleanor::$langs as $k=>$v)
				{
					$values['title'][$k]=isset($_POST['title'][$k]) ? (string)$_POST['title'][$k] : '';
					$values['descr'][$k]=isset($_POST['descr'][$k]) ? (string)$_POST['descr'][$k] : '';
					$values['startgroup'][$k]=isset($_POST['startgroup'][$k]) ? (string)$_POST['startgroup'][$k] : '';
				}
			}
			else
			{
				$values['title']=isset($_POST['title']) ? (string)$_POST['title'] : '';
				$values['descr']=isset($_POST['descr']) ? (string)$_POST['descr'] : '';
				$values['startgroup']=isset($_POST['startgroup']) ? (string)$_POST['startgroup'] : '';
			}
			$protected=false;
			if(is_int($id))
			{
			...
			if(!$protected)
			{
				...
				$values['eval_load']=isset($_POST['eval_load']) ? (string)$_POST['eval_load'] : '';
				$values['eval_save']=isset($_POST['eval_save']) ? (string)$_POST['eval_save'] : '';
			}
		}
		else
			$bypost=false;
