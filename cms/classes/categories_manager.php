...
class Categories_Manager extends Categories
{
	...
	public function Show()
	{
		...
		elseif(isset($_GET[$this->pp.'delete']))
		{
			...
			do
			{
				...
				Eleanor::$Db->Delete(P.'drafts','`key`=\''.get_class($this).'-'.Eleanor::$Login->Get('id').'-'.$id.'\' LIMIT 1');

				Eleanor::$Cache->Engine->DeleteByTag($this->table);
		...
		elseif(isset($_GET[$this->pp.'do']))
			switch($_GET[$this->pp.'do'])
			{
				...
				case'draft':
					...
					Eleanor::$Db->Replace(P.'drafts',array('key'=>get_class($this).'-'.Eleanor::$Login->Get('id').'-'.$id,'value'=>serialize($_POST)));
	...
	protected function AddEdit($id,$errors=array())
	{
		...
		{
			$R=Eleanor::$Db->Query('SELECT `value` FROM `'.P.'drafts` WHERE `key`=\''.get_class().'-'.Eleanor::$Login->Get('id').'-'.$id.'\' LIMIT 1');
