	...
	switch($_GET['do'])
	{
		case'recovernames':
			...
			if($cnt>0)
			{
				...
				while($error=$R->fetch_assoc())
				{
					$error['data']=(array)unserialize($error['data']);
					$error['options']=(array)unserialize($error['options']);

					$error['_aswap']=empty($error['options']['total']) ? false : $Eleanor->Url->Construct(array('swap'=>$error['id']));
					$error['_aedit']=$error['status'] ? false : $Eleanor->Url->Construct(array('edit'=>$error['id']));
					$error['_adel']=$Eleanor->Url->Construct(array('delete'=>$error['id']));

					$items[$error['id']]=array_slice($error,1);
				}
			}
			$links=array(
	...
	if(Eleanor::$our_query)
	{
		...
		if($error=$R->fetch_assoc())
		{
			...
			else
			{
				...
				$error['options']=unserialize($error['options']);
				$error=array(
					'free'=>1,
					'!nextrun'=>'FROM_UNIXTIME('.Tasks::CalcNextRun(array(),$do).')',
					'maxrun'=>1,
					'alreadyrun'=>0,
					'status'=>$error['options']['total']>0,
					'run_month'=>'*',
					'run_day'=>'*',
					'run_hour'=>'*',
					'run_minute'=>'*',
			...
			Eleanor::$Db->Update(P.'tasks',$error,'`id`='.$id.' AND `name`=\'recovernames\' LIMIT 1');
			if($error['status'])
				Tasks::UpdateNextRun();
		}
	}
