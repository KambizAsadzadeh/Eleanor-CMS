...
switch($event)
{
	case'search':
		...
		if($groups)
		{
			$R3=Eleanor::$Db->Query('SELECT `id`,`title_l` `title`,`style` FROM `'.P.'groups` WHERE `id`'.Eleanor::$Db->In($groups));
			$tosort=$groups=array();
...
{global$Eleanor,$title;
	...
	else
	{
		...
		$values['innertitle']=$values['title']=$values['text']=Eleanor::$vars['multilang'] ? array_fill_keys(array_keys(Eleanor::$langs),'') : '';
		if(Eleanor::$vars['multilang'])
		{
			$values['_onelang']=true;
