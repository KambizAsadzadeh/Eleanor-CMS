...
if(isset($_GET['do']))
	switch($_GET['do'])
	{
		...
		case'resort':
			...
			{
				$R=Eleanor::$Db->Query('SELECT `id`,`parents` FROM `'.$mc['t'].'` WHERE `id`='.(int)$_GET['parent'].' LIMIT 1');
		...
		case'draft':
			...
			Eleanor::$Db->Replace(P.'drafts',array('key'=>$mc['n'].'-'.Eleanor::$Login->Get('id').'-'.$id,'value'=>serialize($_POST)));
...
{
	$id=(int)$_GET['delete'];
	$R=Eleanor::$Db->Query('SELECT `parents`,`pos` FROM `'.$mc['t'].'` LEFT JOIN `'.$mc['tl'].'` USING(`id`) WHERE `id`='.$id.' AND `language` IN (\'\',\''.Language::$main.'\') LIMIT 1');
		...
		if(!$errors)
		{
			...
			foreach($a as $k=>&$v)
				if(isset($Eleanor->sc[$k]))
					$values[$k]['value']=$v;
			$R=Eleanor::$Db->Query('SELECT `language`,`title`,`url`,`params` FROM `'.$mc['tl'].'` WHERE `id`='.$id);
			while($temp=$R->fetch_assoc())
...
{global$Eleanor;
	...
	unset($values['title'],$values['url'],$values['params']);
	...
	else
	{
		...
		$values=array('id'=>array(),'language'=>array(),'title'=>array(),'url'=>array(),'params'=>array());
		foreach($langs as &$v)
		{
			$values['id'][]=$id;
			$values['language'][]=$v;
