...
$Eleanor->module['config']=$config=include$Eleanor->module['path'].'config.php';
Eleanor::$Template->queue[]=$config['admintpl'];
...
if(isset($_GET['do']))
	switch($_GET['do'])
	{
		...
		case'draft':
			...
			Eleanor::$Db->Replace(P.'drafts',array('key'=>$config['n'].'-'.Eleanor::$Login->Get('id').'-'.$id,'value'=>serialize($_POST)));
...
{global$Eleanor,$title;
	...
	if($cnt>0)
	{
		$R=Eleanor::$Db->Query('SELECT `id`,`date_from`,`date_till`,`status`,`from`,`to` FROM `'.$mc['t'].'` INNER JOIN `'.$mc['tl'].'` USING(`id`)'.$where.' ORDER BY `'.$sort.'` '.$so.' LIMIT '.$offset.', '.$pp);
...
{global$Eleanor,$title;
		...
		if(!$errors)
		{
			...
			foreach($a as $k=>&$v)
				if(isset($Eleanor->sc[$k]))
					$values[$k]['value']=$v;
			$R=Eleanor::$Db->Query('SELECT `language`,`from`,`regexp`,`to`,`url`,`params` FROM `'.$mc['tl'].'` WHERE `id`='.$id);
	...
	{
		$R=Eleanor::$Db->Query('SELECT `value` FROM `'.P.'drafts` WHERE `key`=\''.$mc['n'].'-'.Eleanor::$Login->Get('id').'-'.$id.'\' LIMIT 1');
...
{global$Eleanor;
	...
	unset($values['to'],$values['from'],$values['regexp'],$values['url'],$values['params']);
	...
	foreach($lvalues['url'] as $k=>$v)
	{
		...
		if(!$v and !$lvalues['title'][$k] and (in_array($k,$langs) or $ml==$k))
	...
	Eleanor::$Db->Delete(P.'drafts','`key`=\''.$mc['n'].'-'.Eleanor::$Login->Get('id').'-'.$id.'\' LIMIT 1');
	if($id)
	{
	...
	else
	{
		...
		$values=array('id'=>array(),'language'=>array(),'from'=>array(),'to'=>array(),'url'=>array(),'params'=>array());
		foreach($langs as &$v)
		{
			$values['id'][]=$id;
			$values['language'][]=$v;
