...
if($Eleanor->module['uris']['news']!=$Eleanor->module['name'])
	$Eleanor->Url->SetPrefix(array('lang'=>Language::$main==LANGUAGE ? false : Eleanor::$langs[Language::$main]['uri'],'module'=>Eleanor::$vars['prefix_free_module']==$Eleanor->module['id'] ? false : $Eleanor->module['uris']['news']));
	...
	switch($d)
	{
		...
		case'my':
			if(Eleanor::$vars['publ_add'])
			{
				...
				{
					$where='`author_id`='.Eleanor::$Login->Get('id');
					$cntf='author_id';
				}
				else
				{
...
elseif($id or $puri)
{
	...
	$R=Eleanor::$Db->Query('SELECT `id`,`cats`,IF(`pinned`=\'0000-00-00 00:00:00\',`date`,`pinned`) `date`,`author`,`author_id`,`show_sokr`,`r_average`,`r_total`,`r_sum`,`status`,`reads`,`comments`,`tags`,`uri`,`title`,`announcement`,`text`,`document_title`,`meta_descr`,UNIX_TIMESTAMP(`last_mod`) `last_mod`,`voting` FROM `'.$mc['t'].'` INNER JOIN `'.$mc['tl'].'` USING(`id`) WHERE `language`IN(\'\',\''.Language::$main.'\') AND '.$where.' LIMIT 1');
	if(!$a=$R->fetch_assoc())
		return ExitPage();
	...
	$Eleanor->module['pub_tags']=$a['tags']=$a['tags'] ? explode(',,',trim($a['tags'],',')) : array();
	$Eleanor->module['pub_id']=$a['id'];
...
elseif($cid or $curls)
{
	...
	if($category['document_title'])
		$title=$category['document_title'];
	else
