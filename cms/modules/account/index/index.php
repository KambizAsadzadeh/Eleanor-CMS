...
if($user or $id)
{
	...
	{
		if($id)
			$R=Eleanor::$Db->Query('SELECT `id`,`u`.`full_name`,`u`.`name`,`integration`,`groups`,`u`.`register`,`u`.`last_visit`,`u`.`language`,`u`.`timezone`,`e`.* FROM `'.USERS_TABLE.'` `u` INNER JOIN `'.P.'users_extra` `e` USING (`id`) INNER JOIN `'.P.'users_site` `s` USING(`id`) WHERE `id`='.$id.' LIMIT 1');
		else
			$R=Eleanor::$Db->Query('SELECT `id`,`u`.`full_name`,`u`.`name`,`integration`,`groups`,`u`.`register`,`u`.`last_visit`,`u`.`language`,`u`.`timezone`,`e`.* FROM `'.USERS_TABLE.'` `u` INNER JOIN `'.P.'users_extra` `e` USING (`id`) INNER JOIN `'.P.'users_site` `s` USING(`id`) WHERE `u`.`name`='.Eleanor::$Db->Escape($user).' LIMIT 1');
	}
	else
	{
		if($id)
		...
		$R=Eleanor::$Db->Query('SELECT `id`,`integration`,`groups`,`full_name`,`name`,`register`,`last_visit`,`language`,`timezone`,`e`.* FROM `'.P.'users_site` INNER JOIN `'.P.'users_extra` `e` USING (`id`) WHERE `id`='.$a['id'].' LIMIT 1');
	}
