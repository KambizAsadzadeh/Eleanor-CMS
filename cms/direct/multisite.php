...
switch($type)
{
	case'check':#Проверка стороннего сайта на логин в текущем сайте
		...
		$r=Eleanor::$Login->IsUser() ? Eleanor::JsVars(array('title'=>Eleanor::$vars['site_name'],)+Eleanor::$Login->Get(array('id','name'),false),false,true) : 'false';
		$c=isset($_GET['c']) ? (string)$_GET['c'] : '';
		Start();
		header('Access-Control-Allow-Origin: *');
		echo'try{'.$c.'('.$r.')}catch(e){}';
	break;
	case'get-login':#Получение логина с текущего сайта для возможности логина на стороннем
		if(Eleanor::$Login->IsUser())
		{
			Eleanor::LoadOptions('multisite');
			$r=Eleanor::$Login->Get(array('id','name'),false);
			$r=array(
				'uid'=>$r['id'],
				'name'=>$r['name'],
			);
	...
	case'pre-jump':#Подготовка к прыжку с текущего на сторонний
		if(Eleanor::$Login->IsUser())
		{
			...
			if(!isset($ms[$sn]))
				return Error();
			$d=$ms[$sn];
			$data=Eleanor::$Login->Get(array('id','name'),false);
			$data=array(
