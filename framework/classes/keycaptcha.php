...
/** Адаптер для KeyCaptcha. Для того, чтобы скрипт работал, необходимо перейти на http://www.keycaptcha.com/?refid=79215
 * и получить PRIVATE KEY и USER ID, которые необходимо вставить в переменные $private_key и $user_id класса KeyCaptcha */
class KeyCaptcha extends Eleanor\BaseClass implements Eleanor\Interfaces\Captcha
{
	public static
		/** @var string Жирное численно-буквенное значение указанно на вкладке Installation instruction в пункте 3 */
		$private_key='',

		/** @var int Жирное числовое значение указанное на вкладке Installation instruction в пункте 2 */
		$user_id=0;
	...
	public static function Check($post=null)
	{
		...
		return isset($post['capcode']) && $KC->check_result($post['capcode']);
	}
}

#Содержимое https://www.keycaptcha.com/media/code/keycaptcha.php с исправленными мною ошибками
...
class KeyCAPTCHA_CLASS
{
	...
	public $p_kc_user_id = 'USER_ID_HERE';#Eleanor: private -> public
	private $c_kc_keyword = 'accept';
	private $p_kc_visitor_ip = '';
	private $p_kc_session_id = '';
	#Eleanor: удаление 2х неиспользуемых свойств
	...
	public function check_result($response)
	{
		...
		if ($kc_vars[0] == md5($this->c_kc_keyword . $kc_vars[1] . $this->p_kc_private_key . $kc_vars[2]))
		{
			if (stripos($kc_vars[2], "http://") !== 0)
			{
				...
				$kc_var_time = preg_split('#[/ :]#', $kc_vars[2]);#Eleanor: split -> preg_split
