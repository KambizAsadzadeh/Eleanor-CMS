...
class Files extends Eleanor\BaseClass
{
		...
		*/
		...
		foreach($links as $k=>$file)
		{
			...
			if(Eleanor\W ? is_file($fulltemp) && readlink($fulltemp)!=$fulltemp : is_link($fulltemp))
			{
				...
				if(Eleanor\W and strpos($orig,':')!==1 or !Eleanor\W and strpos($orig,'/')!==0)
	...
	public static function Delete($path,$nocheck=false)
	{
		if(is_dir($path))
		{
			...
			foreach($entries as $entry)
				if(!static::Delete($path.DIRECTORY_SEPARATOR.$entry,true))
					return true;
	...
	public static function GetSize($path,$filter=null)
	{
		if(is_link($path))
			return 0;
